...
impl OcrProviderChain {
    ...
    pub async fn recognize_for_login(&self, image_bytes: &[u8]) -> AppResult<String> {
        let mut last_error = None;
        for _ in 0..self.native_attempts {
            match self.native.recognize(image_bytes).await {
                Ok(text) if !text.trim().is_empty() => return Ok(text),
                Ok(_) => last_error = Some("原生 OCR 返回空结果".to_string()),
                Err(err) => last_error = Some(err.to_string()),
            }
        }
        match self.worker.recognize(image_bytes).await {
